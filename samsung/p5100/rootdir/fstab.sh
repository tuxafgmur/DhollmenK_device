...
/sbin/busybox blkid -s TYPE /dev/block/mmcblk0p9 | /sbin/busybox grep -q ext4
if [ "$?" = "0" ]; then
	echo '/dev/block/platform/omap/omap_hsmmc.1/by-name/FACTORYFS  /system      ext4    ro,noatime,nodiratime,barrier=0,discard,noauto_da_alloc,journal_async_commit,data=writeback            wait' >> $STABFILE
else
...
/sbin/busybox blkid -s TYPE /dev/block/mmcblk0p10 | /sbin/busybox grep -q ext4
if [ "$?" = "0" ]; then
	echo '/dev/block/platform/omap/omap_hsmmc.1/by-name/DATAFS     /data        ext4    noatime,nodiratime,nosuid,nodev,barrier=0,discard,noauto_da_alloc,journal_async_commit,data=writeback  wait,check,encryptable=footer' >> $STABFILE
else
...
/sbin/busybox blkid -s TYPE /dev/block/mmcblk0p7 | /sbin/busybox grep -q ext4
if [ "$?" = "0" ]; then
	echo '/dev/block/platform/omap/omap_hsmmc.1/by-name/CACHE      /cache       ext4    noatime,nodiratime,nosuid,nodev,barrier=0,discard,noauto_da_alloc,journal_async_commit,data=writeback  wait,check' >> $STABFILE
else
