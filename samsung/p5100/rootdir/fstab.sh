...
if [ "$?" = "0" ]; then
	...
	echo '/dev/block/platform/omap/omap_hsmmc.1/by-name/FACTORYFS  /system      ext4    noatime,nodiratime,nosuid,nodev,barrier=0,discard,noauto_da_alloc,journal_async_commit,data=writeback  wait' >> $STABFILE
fi
...
if [ "$?" = "0" ]; then
	...
	echo '/dev/block/platform/omap/omap_hsmmc.1/by-name/HIDDEN     /preload     ext4    noatime,nodiratime,nosuid,nodev,barrier=0,discard,noauto_da_alloc,journal_async_commit,data=writeback  wait,check' >> $STABFILE
fi
